"""IntegrAO one-omic-at-a-time feature sweep (README 9.3): target omic at `ratio`,
...
def main() -> None:
    ...
    model = IntegrAOModel(cv_spec=cv_spec, feature_selection_spec=build_k_fs_spec(k_per_omic=100),
    ...
    result = model.evaluate_on_feature_ratio_one_omic(
        ...
        notes="IntegrAO one-omic-at-a-time feature sweep (README 9.3) via Slurm array")

    run_name = (f"integrao__{args.mode}__omic{args.target_omic}__nb{args.neighbor_size}"
                f"__emb{args.embedding_dims}__ratio{f2tag(args.ratio)}"
    ...
    run_dir = save_benchmark_result(
        ...
        source_files=[os.path.join(os.path.dirname(__file__), "integrao_model.py"),
