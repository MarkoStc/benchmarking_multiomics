"""One SLURM array task: fixed LogRegEarlyModel HPs, one target block-missingness
rate (README section 8.2 random block missingness).

Uses the *local* framework copy in ../evaluation_ext, which fixes
evaluate_on_missingness_ratio (the upstream copy referenced an undefined `ms` and
prepared data from the unmasked dataset). We pre-import base/common/results_util
from evaluation_ext so that when sk_bench later does its own
`sys.path.insert(0, ../../evaluation)`, the already-cached (fixed) modules win.
"""
...
import base  # noqa: F401,E402  (force evaluation_ext/base into sys.modules first)
...
def main() -> None:
    ...
    result = model.evaluate_on_missingness_ratio(
        ...
        notes="logreg_early fixed-HP block-missingness sweep (README 8.2) via Slurm array",
    )

    run_name = (
        f"logreg_early__C{f2tag(args.downstream_c)}__k{args.k_per_omic}"
