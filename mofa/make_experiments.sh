#!/bin/bash
# Generate MOFA experiment files for all datasets and axes.
#
# HP search grid (MOFA specific):
#   n_latent     : 5 15 30
#   downstream_c : 0.01 0.1 1.0 10.0
#
...
for DATASET in "${DATASETS[@]}"; do
  DATASET_PATH="${DATA_ROOT}/${DATASET}.compat.joblib"
      ...
      for c in "${CS[@]}"; do
        for k in "${KS[@]}"; do
          for frac in "${FRACS[@]}"; do
            echo "python run_missing.py ${COMMON} --omics ${ALL_OMICS} --n-latent ${nl} --downstream-c ${c} --k-per-omic ${k} --include-non-intersection-frac ${frac} --random-state ${seed} --output-root results/${DATASET}/missing" >> "$OUTFILE"
          done
        done
      done
    done
    ...
    for nl in "${NLS[@]}"; do
      for c in "${CS[@]}"; do
        for ratio in "${RATIOS[@]}"; do
          echo "python run_ratio.py ${COMMON} --omics ${ALL_OMICS} --n-latent ${nl} --downstream-c ${c} --ratio-per-omic ${ratio} --include-non-intersection-frac 1.0 --random-state ${seed} --output-root results/${DATASET}/ratio" >> "$OUTFILE"
        done
      done
    done
  done
  ...
  for seed in "${SEEDS[@]}"; do
    for combo in "${COMBOS[@]}"; do
      echo "python run_nomics.py ${COMMON} --omics ${combo} --n-latent ${NL_DEF} --downstream-c ${C_DEF} --k-per-omic ${K_DEF} --include-non-intersection-frac 1.0 --random-state ${seed} --output-root results/${DATASET}/nomics" >> "$OUTFILE"
    done
  done
  ...
  for seed in "${SEEDS[@]}"; do
    for n in "${NPATS[@]}"; do
      echo "python run_npatients.py ${COMMON} --omics ${ALL_OMICS} --n-latent ${NL_DEF} --downstream-c ${C_DEF} --k-per-omic ${K_DEF} --n-patients ${n} --include-non-intersection-frac 1.0 --random-state ${seed} --output-root results/${DATASET}/npatients" >> "$OUTFILE"
    done
  done
