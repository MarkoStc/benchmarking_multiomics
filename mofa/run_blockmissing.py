"""One SLURM array task: fixed MOFA HPs, one target block-missingness rate
(README 8.2). Pre-imports the fixed framework copy in ../evaluation_ext."""
...
def main() -> None:
    ...
    model = MOFAModel(cv_spec=cv_spec, feature_selection_spec=fs_spec,
    ...
    result = model.evaluate_on_missingness_ratio(
        ...
        notes="MOFA fixed-HP block-missingness sweep (README 8.2) via Slurm array",
    )

    run_name = (
        f"mofa__nl{args.n_latent}__C{f2tag(args.downstream_c)}__k{args.k_per_omic}"
    ...
    run_dir = save_benchmark_result(
        ...
        source_files=[
            os.path.join(os.path.dirname(__file__), "mofa_model.py"),
