"""One SLURM array task: fixed LogRegLateModel HPs, one target block-missingness
...
def main() -> None:
    ...
    result = model.evaluate_on_missingness_ratio(
        ...
        notes="logreg_late fixed-HP block-missingness sweep (README 8.2) via Slurm array",
    )

    run_name = (
        f"logreg_late__C{f2tag(args.downstream_c)}__k{args.k_per_omic}"
