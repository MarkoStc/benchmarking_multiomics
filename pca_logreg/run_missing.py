"""One SLURM array task: fixed PCALogRegModel HPs, one missing value."""
...
from sk_bench import PCALogRegModel as Model, dump_fold_artifacts
...
def main() -> None:
    ...
    p.add_argument("--include-non-intersection-frac", type=float, required=True)
    ...
    p.add_argument("--output-root", default="results_pca_logreg_missing")
    args = p.parse_args()
    ...
    result = model.evaluate_on_missing_patients(
        dataset=dataset,
        omics=omics,
        include_non_intersection_fracs=[args.include_non_intersection_frac],
        missing_policy=args.missing_policy,
        notes="pca fixed-HP missing-patient sweep via Slurm array",
    )

    run_name = (
        f"pca"
        f"__nc{args.n_components}__C{f2tag(args.downstream_c)}"
        + f"__k{args.k_per_omic}__frac{f2tag(args.include_non_intersection_frac)}__seed{args.random_state}"
    )
    ...
    run_dir = save_benchmark_result(
        ...
        source_files=[
            os.path.join(os.path.dirname(__file__), "run_missing.py"),
