"""One SLURM array task: fixed PCALogRegModel HPs, one npatients value."""
...
def main() -> None:
    ...
    p.add_argument("--output-root", default="results_pca_logreg_npatients")
    args = p.parse_args()
    ...
    result = model.evaluate_on_n_patients(
        ...
        feature_selection_spec=fs_spec,
        notes="pca fixed-HP n-patients sweep via Slurm array",
    )

    run_name = (
        f"pca"
        f"__nc{args.n_components}__C{f2tag(args.downstream_c)}"
        + f"__k{args.k_per_omic}__n{args.n_patients}__seed{args.random_state}"
    )
    ...
    run_dir = save_benchmark_result(
        ...
        source_files=[
            os.path.join(os.path.dirname(__file__), "run_npatients.py"),
