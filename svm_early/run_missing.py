"""One SLURM array task: fixed SVMEarlyModel HPs, one missing value."""
...
from sk_bench import SVMEarlyModel as Model, dump_fold_artifacts
...
def main() -> None:
    ...
    p.add_argument("--output-root", default="results_svm_early_missing")
    args = p.parse_args()
    ...
    result = model.evaluate_on_missing_patients(
        ...
        notes="svm_early fixed-HP missing-patient sweep via Slurm array",
    )

    run_name = (
        f"svm_early"
        f"__C{f2tag(args.svm_c)}__ker{args.kernel}"
