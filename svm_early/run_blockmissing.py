"""One SLURM array task: fixed SVMEarlyModel HPs, one target block-missingness
rate (README 8.2). Uses the fixed framework copy in ../evaluation_ext (pre-imported
so it wins the sys.modules cache before sk_bench re-inserts ../../evaluation)."""
...
def main() -> None:
    ...
    result = model.evaluate_on_missingness_ratio(
        ...
        notes="svm_early fixed-HP block-missingness sweep (README 8.2) via Slurm array",
    )

    run_name = (
        f"svm_early__C{f2tag(args.svm_c)}__ker{args.kernel}__k{args.k_per_omic}"
