#!/bin/bash
# SECOND-USER submitter: runs under the FRIEND's account (higher fair-share).
# Owns svm_early, pca_logreg, logreg_late ONLY. logreg_early + all core reruns
# stay on the primary account. Uses squeue -u $USER so the per-user QOS cap and
# priority auto-apply to whoever runs it. Own state file (no collision with the
# primary submitter). See CLAUDE.md for the division of labor.
set -u

ROOT=/work/scitas-share/FellayMultiOmic/code/full-test-pipeline
STATE="$ROOT/submit_daemon_her.state"
CAP=1801
MARGIN=40
MIN_SLICE=10
POLL=90

MODELS=(svm_early pca_logreg logreg_late)
declare -A SLURM=(
